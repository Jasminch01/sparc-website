...
const Whatwedo = () => {
  return (
    <div className="bg-[#36133B] py-12 md:py-15 lg:py-20 relative">
      <div className="max-w-7xl mx-auto flex flex-col  gap-8 md:gap-10 pb-0 lg:pb-10 px-5 lg:px-10 xl:px-0">
        {/* Header Section */}
        <div className="text-center text-white space-y-3 md:space-y-4">
          <h2 className={`${poppins.className} font-black text-3xl sm:text-4xl md:text-5xl lg:text-4xl`}>
            WHAT WE DO
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5 lg:gap-6">
          {whatwedoes.map((wwd, index) => (
            <div key={index}
              ...
              <h3 className={`text-white text-sm sm:text-base md:text-lg font-semibold leading-snug ${poppins.className}`}>
                {wwd.title}
              </h3>
            </div>
          ))}

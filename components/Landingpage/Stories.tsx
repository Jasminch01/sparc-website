  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Query for featured stories only
        const storiesQuery = `*[_type == "indispeakStories" && isFeatured == true] | order(writtenOn desc) [0...3] {
          _id,
          title,
          writtenOn,
          des,
        ...
        // Map stories with type and format the data
        const storiesWithType = storiesData.map((story: Story) => ({
          ...story,
          type: "story" as const,
          date: story.writtenOn, // Map writtenOn to date for consistency
        }));

        setCombinedData(storiesWithType);
      } catch (error) {
              ...
              {combinedData.slice(0, 3).map((item, index) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {getDescription(item)
                        ? getDescription(item).slice(0, 190)
